...
import { addProduct, deleteProduct, getCategories, getUserProducts, updateProduct, uploadFile } from '../../api';

export default function ProductsTab({ activeTab }: { activeTab: number }) {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      setMyProducts(userProductsResponse.data);
  ...
  const handleEditProductClick = (e: React.MouseEvent<HTMLLIElement>) => {
  ...
  const handleGeneralChange = (e: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement | HTMLSelectElement>) => {
  ...
  const handleUpdateProductSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (!selectedProduct?._id) {
        toast.warning('Select a product to update');
        setLoading(false);
        return;
      }
  ...
  const handleRemoveProduct = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    if (!selectedProduct?._id) {
      toast.warning('Select a product to remove');
      setLoading(false);
      return;
    }
                    ...
                    return (
                      <li
                        ...
                        className={`list-group-item d-flex justify-content-between align-items-center list-custom ${
                          selectedProduct && (product._id === selectedProduct._id ? 'active' : '')
                        }`}
                      >
                        {product.name}
                        <span
                          className={`badge ${
                            product.quantity! > 0 ? (product.quantity! > 5 ? 'bg-primary' : 'bg-warning') : 'bg-danger'
                          } rounded-pill`}
                        >
                          {product.quantity}
                        </span>
          ...
          {editingProduct ? (
            <div>
              ...
              <form onSubmit={handleUpdateProductSubmit}>
                ...
                <div className='mb-3'>
                  <label className='form-label' htmlFor='prod-pic'>
                    An attractive picture of your product
                  </label>
                  <input
                ...
                <button type='submit' className='btn btn-outline-danger border border-dark' form='deleteProdForm'>
                  Remove this product
                </button>
              </form>
            ...
            <div>
              ...
              <form onSubmit={handleAddProductSubmit}>
                {/* name, desc, category, image_url, quantity, price */}
                <div className='mb-3'>
                  <label className='form-label' htmlFor='prod-name-new'>
                    Name
                  </label>
                  <input
                    onChange={handleGeneralChange}
                ...
                <div className='mb-3'>
                  <label className='form-label' htmlFor='prod-pic-new'>
                    An attractive picture of your product
                  </label>
                  <input
                    onChange={handleProductFileChange}
                ...
                <div className='mb-3'>
                  <label className='form-label' htmlFor='prod-desc-new'>
                    Description
                  </label>
                  <textarea
                    value={newProduct?.description || ''}
                ...
                <div className='mb-3'>
                  <label className='form-label' htmlFor='category-new'>
                    Category
                  </label>
                  <select
                    onChange={handleGeneralChange}
                ...
                <div className='mb-3'>
                  <label className='form-label' htmlFor='quantity-new'>
                    Quantity
                  </label>
                  <input
                    onChange={handleGeneralChange}
                ...
                <div className='mb-3'>
                  <label className='form-label' htmlFor='price-new'>
                    Price (&euro;)
                  </label>
                  <input
                    onChange={handleGeneralChange}
