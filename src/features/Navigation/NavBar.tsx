      ...
      <nav className='navbar navbar-expand-lg navbar-light bg-warning'>
        <div className='container-fluid'>
          <a className='navbar-brand' href='/'>
            <img src='/images/multistore-navbar-logo-transparent.png' height='35px' alt='multistore logo' />
          </a>
          <button
          ...
          <div className='collapse navbar-collapse' id='navbarSupportedContent'>
            <ul className='navbar-nav me-auto mb-2 mb-lg-0'>
              <li className='nav-item dropdown'>
                <button
                  onFocus={updateCategories}
                  className='nav-link dropdown-toggle btn btn-link shadow-none'
                  id='navbarDropdown'
              ...
              <li className='nav-item' style={{ position: 'relative' }}>
                ...
                <div
                  className={
                    (firstThreeFromSearch.length ? 'd-block' : 'd-none') + ' m-0 p-0 custom-width border bg-white'
                  }
                      ...
                      {firstThreeFromSearch.map((product, index) => {
                        return (
                          <a
                            key={product._id}
                            className={
                              'p-1 list-group-item list-group-item-action list-group-item-light rounded-0 border-0' +
                              (index === 0 ? ' active' : '')
                            }
                            id={`list-${index}-list`}
                            data-bs-toggle='list'
                          ...
                          >
                            {product.name!.length > 15 ? product.name!.substring(0, 16) + '...' : product.name}
                          </a>
                        );
                      })}
                    </div>
                        ...
                        return (
                          ...
                          >
                            <div className='container-fluid p-0 d-flex flex-column'>
                              ...
                              <div
                                className='text-wrap'
                                style={{ height: '200px', fontSize: '0.8rem', overflowY: 'hidden' }}
                              >
                                <Link
              ...
              {loggedInUser ? (
                <>
                  <li className='d-flex align-items-center custom-margin nav-item'>
                    <div
                      ...
                      style={{ backgroundImage: `url(${loggedInUser.profile_picture})` }}
                    ></div>
                    <span className='me-3'>Hey {loggedInUser.username}</span>
                  </li>
                  <li className='nav-item position-relative' id='cart'>
                    <IconContext.Provider value={{ color: 'black', size: '3rem' }}>
                      <NavLink to='/cart'>
                        <MdShoppingCart />
                      </NavLink>
                      <span className='position-absolute start-50 top-0 translate-middle'>{basketQuantity}</span>
                    </IconContext.Provider>
                  </li>
                  <li className='nav-item'>
                    <NavLink className='nav-link' to='/'>
                      <button
                        className='btn btn-outline-danger border-0 pb-0 pt-0'
                        type='button'
                        onClick={logoutUser}
                        id='logout'
                      >
                ...
                <>
                  <li className='nav-item'>
                    <NavLink className='nav-link custom-margin' to='/login'>
                      Login
                    </NavLink>
                  </li>
                  <li className='nav-item'>
