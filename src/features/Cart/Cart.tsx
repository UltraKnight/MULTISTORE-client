...
export default function Cart({
  ...
  const handleAddRemove = async (
    ...
    if (quantity === availableQt && e.currentTarget.name === 'frmAdd') {
      toast.warning('reached the maximum available');
      setIsLoading(false);
      return;
    }

    const product = { product: id, quantity: e.currentTarget.name === 'frmRemove' ? 0 : 1 };
    try {
      if (e.currentTarget.name === 'frmAdd') {
      ...
      const response = await loggedin(); // get updated user data to check if any of the products were removed by the author
      setBasketQuantity(response?.data?.cart?.reduce((acc, item) => acc + item.quantity, 0) || 0);
    ...
    <div className='table-responsive-md'>
            ...
            return (
              <tr key={item._id} className='align-middle fs-6'>
                {item.product.quantity! > 0 ? (
                  ...
                  <td className='fw-bold text-start'>
                    This product is no longer available, please remove from your cart
                  </td>
                )}
                <td>
                ...
                <td>
                  <form name='frmRemove' onSubmit={(e) => handleAddRemove(e, item.product?._id ?? null)}>
                ...
                <td>{item.product ? <>&euro; {(item.quantity * item.product.price!).toFixed(2)}</> : null}</td>
              </tr>
            );
          })}
          <tr>
            ...
            {canFinish ? (
              <td>
                &euro;{' '}
                {user.cart
                  .reduce((accumulator, curr) => accumulator + curr.quantity * curr.product.price!, 0)
                  .toFixed(2)}
              </td>
            ) : null}
          </tr>
      ...
      {user.emailConfirmed ? null : (
        <p className='text-center'>
          Please, confirm your email before proceed. <Link to='/profile'>Go to profile</Link>
        </p>
      )}
    </div>
  ) : (
