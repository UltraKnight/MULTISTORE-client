...
export default function Profile() {
  ...
  const fullNameRef = useRef<HTMLSpanElement | null>(null);
  const storeNameRef = useRef<HTMLSpanElement | null>(null);
  const usernameRef = useRef<HTMLSpanElement | null>(null);
  const emailRef = useRef<HTMLInputElement | null>(null);
  const bilFirstNameRef = useRef<HTMLSpanElement | null>(null);
  const bilLastNameRef = useRef<HTMLSpanElement | null>(null);
  const bilAddress1Ref = useRef<HTMLSpanElement | null>(null);
  const bilAddress2Ref = useRef<HTMLSpanElement | null>(null);
  const bilCityRef = useRef<HTMLSpanElement | null>(null);
  const bilStateRef = useRef<HTMLSpanElement | null>(null);
  const bilCountryRef = useRef<HTMLSpanElement | null>(null);
  const bilPostcodeRef = useRef<HTMLSpanElement | null>(null);
  const bilPhoneRef = useRef<HTMLSpanElement | null>(null);
  const shipFirstNameRef = useRef<HTMLSpanElement | null>(null);
  const shipLastNameRef = useRef<HTMLSpanElement | null>(null);
  const shipAddress1Ref = useRef<HTMLSpanElement | null>(null);
  const shipAddress2Ref = useRef<HTMLSpanElement | null>(null);
  const shipCityRef = useRef<HTMLSpanElement | null>(null);
  const shipStateRef = useRef<HTMLSpanElement | null>(null);
  const shipCountryRef = useRef<HTMLSpanElement | null>(null);
  const shipPostcodeRef = useRef<HTMLSpanElement | null>(null);
  const shipPhoneRef = useRef<HTMLSpanElement | null>(null);
  const isShipEqualsRef = useRef<HTMLInputElement | null>(null);
  ...
  const handleBasicKeyPress = (e: React.KeyboardEvent<HTMLSpanElement>) => {
    if (e.currentTarget.innerHTML.length > 200) {
      e.preventDefault();
    }
  ...
  const handleBillingKeyPress = (e: React.KeyboardEvent<HTMLSpanElement>) => {
    if (e.currentTarget.innerHTML.length > 200) {
      e.preventDefault();
    }
  ...
  const handleShippingKeyPress = (e: React.KeyboardEvent<HTMLSpanElement>) => {
    if (e.currentTarget.innerHTML.length > 200) {
      e.preventDefault();
    }
  ...
  const handleBasicClick = async () => {
    if (editableBasic) {
      ...
      try {
        const response = await updateProfile({ fullName, storeName, username });
    ...
    if (editableBilling) {
      ...
      try {
        const response = await updateProfile({
          billing: { firstName, lastName, address1, address2, city, state, country, postcode, phone },
        });
        ...
        if (copyToShip) {
          ...
          await updateProfile({
            shipping: {
              ...
              postcode,
              phone,
              phoneConfirmed: !!user.billing?.phoneConfirmed,
            },
          });
  ...
  const handlePostcodeInput = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (
      /^([0-9]{5}|[a-zA-Z][a-zA-Z ]{0,49})$/.test(e.code) &&
      e.code !== 'Backspace' &&
      e.code !== 'ArrowLeft' &&
      e.code !== 'ArrowRight' &&
      e.code !== 'Delete' &&
  ...
  const handleShippingClick = async () => {
    if (editableShipping) {
      ...
      try {
        const response = await updateProfile({
          shipping: { firstName, lastName, address1, address2, city, state, country, postcode, phone },
        });
        setEditableShipping(false);
        toast.success(response.data);
      } catch (error) {
    ...
    <div className='container-fluid mt-3'>
      <div className='row'>
        ...
        <div className='col-lg-4 col-md-5'>
          ...
          <div className='d-inline float-end'>
            {editableBasic ? (
              <>
                <button
                  onMouseUp={handleBasicClick}
                  style={{ fontSize: '1.25rem' }}
                  className='text-dark btn btn-transparent p-0 m-0 shadow-none ms-3'
                >
                  <FiSave />
                </button>
                <button
                  onMouseUp={() => {
                    setEditableBasic(false);
                    if (fullNameRef.current && storeNameRef.current) {
                      fullNameRef.current.innerHTML = user.fullName ?? '';
                      storeNameRef.current.innerHTML = user.storeName ?? '';
                    }
                  }}
                  style={{ fontSize: '1.25rem' }}
                  className='btn btn-transparent p-0 m-0 shadow-none ms-3'
                >
                  <FiX />
                </button>
              </>
          ...
          <p>
            ...
            <span
              ref={fullNameRef}
              className={editableBasic ? 'border border-dark' : 'bg-light'}
              onKeyUp={handleBasicKeyPress}
              suppressContentEditableWarning={true}
              contentEditable={`${editableBasic}`}
            >
              {user.fullName}
            </span>
          </p>
        ...
        <div className='col-lg-4 col-md-5'>
          ...
          <div className='d-inline float-end'>
            {editableContact ? (
              <>
                ...
                <button
                  onMouseUp={() => setEditableContact(false)}
                  style={{ fontSize: '1.25rem' }}
                  className='btn btn-transparent p-0 m-0 shadow-none ms-3'
                >
                  <FiX />
          ...
          <p>
            Email: <span style={{ display: editableContact ? 'none' : 'inline' }}>{user.email}</span>
            <small className={user.emailConfirmed ? 'text-success' : 'text-danger'}>
              <br />
          ...
          <form id='form-contact' onSubmit={handleEmailSubmit}>
            <input
              ...
              type='email'
              maxLength={200}
              style={{ display: editableContact ? 'inline' : 'none' }}
            />
          </form>
        </div>
      </div>
      <div className='row'>
        <div className='col-lg-4 offset-lg-3 col-md-5 offset-md-2 mb-3'>
          ...
          <div className='d-inline float-end'>
            {editableBilling ? (
              <>
                ...
                <button
                  onMouseUp={handleCancelBilEditing}
                  style={{ fontSize: '1.25rem' }}
                  className='btn btn-transparent p-0 m-0 shadow-none ms-3'
                >
                  <FiX />
          ...
          <p>
            ...
            <span
              ref={bilFirstNameRef}
              className={editableBilling ? 'border border-dark' : 'bg-light'}
              onKeyUp={handleBillingKeyPress}
              suppressContentEditableWarning={true}
              contentEditable={`${editableBilling}`}
            >
          ...
          <p>
            ...
            <small className={user.billing?.phoneConfirmed ? 'text-success' : 'text-danger'}>
              <br />
              {user.billing?.phoneConfirmed ? 'confirmed' : 'not confirmed'}
            </small>
          </p>
        ...
        <div className='col-lg-4 col-md-5'>
          ...
          <div className='d-inline float-end'>
            {editableShipping ? (
              <>
                ...
                <button
                  onMouseUp={handlelCancelShipEditing}
                  style={{ fontSize: '1.25rem' }}
                  className='btn btn-transparent p-0 m-0 shadow-none ms-3'
                >
                  <FiX />
          ...
          <p>
            First name:{' '}
            <span
              ref={shipFirstNameRef}
              className={editableShipping ? 'border border-dark' : 'bg-light'}
              onKeyUp={handleShippingKeyPress}
              suppressContentEditableWarning={true}
              contentEditable={`${editableShipping}`}
            >
          ...
          <p>
            ...
            <small className={user.shipping?.phoneConfirmed ? 'text-success' : 'text-danger'}>
              <br />
              {user.shipping?.phoneConfirmed ? 'confirmed' : 'not confirmed'}
            </small>
          </p>
        </div>
      </div>
